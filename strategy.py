...
def step_0(player, table):
    if gameSettings.DEBUG_strategy_prints_agents: print("Check step 0")
    decision = 0
...
def step_1(player, table):
    if gameSettings.DEBUG_strategy_prints_agents: print("Check step 1")
    decision = 1
    target_player = player
    result = 0
    ...
    for card in range(len(player.hand)):
        rank = player.knows_rank(table, card)
        if rank is not None:
            # Rank is known - check if all cards with possible colours for this card also are playable
            check_colours = [target_colours[i] for i in range(len(target_ranks)) if target_ranks[i] == rank]
            sufficient_knowledge = True
            for colour in range(len(player.hand_knowledge[card])):
                # Check if the rank to check is eligible for this colour
                if player.hand_knowledge[card][colour][rank - 1] == 1:
                    # Check if all colours for which we consider the rank are playable with this rank
                    if table.deck.colours_in_game[colour] not in check_colours:
                        sufficient_knowledge = False
            if sufficient_knowledge is True:
                found_card = True
                result = card
    if found_card == False:
...
def step_2(player, table):
    if gameSettings.DEBUG_strategy_prints_agents: print("Check step 2")
    decision = 2
...
def step_3(player, table):
    if gameSettings.DEBUG_strategy_prints_agents: print("Check step 3")
    decision = 3
    target_player = table.player_list[(table.total_turn_counter + 1) % gameSettings.player_amount]
    result = ''
    if table.tokens.note_tokens > 0:
...
def step_4(player, table):
    if gameSettings.DEBUG_strategy_prints_agents: print("Check step 4")
    decision = 4
                ...
                if card is not None:
                    if target[0] == card.colour and target[1] == card.value and target_player.knows_card(table, hand.index(card)) is None:
                        if target_player.knows_rank(table, hand.index(card)) is not None:
                            has_target = True
                            # Check if either the card is a rainbow card or rainbow is the only other considered colour
                            if card.colour is not 'rainbow' and not sum(target_player.hand_knowledge[hand.index(card)][table.deck.colours_in_game.index(card.colour)]) <= 2:
                                result = target[0]
                            else:
                                targets = []
...
def step_5(player, table):
    if gameSettings.DEBUG_strategy_prints_agents: print("Check step 5")
    decision = 5
    found_target = False
    if table.tokens.note_tokens > 0:
...
def step_6(player, table):
    if gameSettings.DEBUG_strategy_prints_agents: print("Check step 6")
    decision = 6
    target_player = player
    result = 0
    found_card = False
...
def step_7(player, table):
    if gameSettings.DEBUG_strategy_prints_agents: print("Check step 7")
    decision = 7
    target_player = player
    result = 0
    target_found = False
...
def step_8(player, table):
    if gameSettings.DEBUG_strategy_prints_agents: print("Check step 8")
    decision = 8
    target_player = player
    if table.tokens.note_tokens > 0:
        decision = 80
